...
/*
 * why UINT32_MAX (which "apm.h" has already defined) is undefined if I don't
 * define it here?
 */
...
*/
/*
 * kernel code uses this to dump the result into buffer at usermode.
 * address of the user buffer shouldn't be NULL.
 */
void bn_dump(const bn *n, unsigned int base, void __user *buf)
{
    if (n->size == 0) {
        /* the cast is intent to silence Cppcheck */
        copy_to_user((char *) buf, "0", 2);
        return;
    }

    if (unlikely(n->sign))
        copy_to_user((char *) buf++, "-", 1);

    apm_dump(n->digits, n->size, base, buf);
}
